...
{
  return body_largest_variable_node(var_node);
}
...
{
  struct symbol* struct_sym = symresolver_get_symbol(compiel_proc, struct_name);
  assert(struct_sym && struct_sym->type == SYMBOL_TYPE_NODE);
  struct node* node = struct_sym->data;
  assert(node && node_is_struct_or_union(node));
