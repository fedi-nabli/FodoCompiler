...
// < Preprocessor initializer functions end
...
void preprocessor_make_parentheses_node(struct expressionable* expressionable, void* node_ptr)
{
  ...
  parentheses_node.parenthesis_node.exp = node;
...
struct preprocessor_definition* preprocessor_definition_create(struct preprocessor* preprocessor, const char* name, struct vector* value_vec, struct vector* arguments)
{
...
void preprocessor_number_push_to_function_arguments(struct preprocessor_function_arguments* arguments, int64_t number)
{
  ...
  token.llnum = number;
  preprocessor_token_push_to_function_arguments(arguments, &token);
}
...
int preprocessor_arithmetic(struct compile_process* compiler, long left_operand, long right_operand, const char* op)
{
  ...
  if (!success)
  {
    compiler_error(compiler, "We do not support the operator %s for the preprocessor arithmetic\n", op);
  }
...
{
  return node->type == PREPROCESSOR_EXPRESSION_NODE && S_EQ(node->exp_node.op, "()") && node->exp_node.left_node->type == PREPROCESSOR_IDENTIFIER_NODE;
}
...
{
  if (node->type == PREPROCESSOR_EXPRESSION_NODE && S_EQ(node->exp_node.op, ","))
  {
    preprocessor_evaluate_function_call_argument(compiler, node->exp_node.left_node, arguments);
    preprocessor_evaluate_function_call_argument(compiler, node->exp_node.right_node, arguments);
    return;
  }
  else if (node->type == PREPROCESSOR_PARENTHESES_NODE)
  {
    preprocessor_evaluate_function_call_argument(compiler, node->parenthesis_node.exp, arguments);
    return;
  }
...
void preprocessor_handle_typdef_body_for_non_struct_or_union(struct compile_process* compiler, struct vector* token_vec, struct typedef_type* td, struct vector* src_vec, bool overflow_use_token_vec)
{
...
void preprocessor_handle_typedef_body(struct compile_process* compiler, struct vector* token_vec, struct typedef_type* td, struct vector* src_vec, bool overflow_use_token_vec)
{
  ...
  {
    preprocessor_handle_typdef_body_for_non_struct_or_union(compiler, token_vec, td, src_vec, overflow_use_token_vec);
  }
}
...
void preprocessor_handle_typedef_token(struct compile_process* compiler, struct vector* src_vec, bool overflow_use_token_vec)
{
  ...
  {
    compiler_error(compiler, "The typdef name has to be a valid identifier");
  }
  ...
  // to the output so it van be found
  ...
  preprocessor_definition_create_typedef(preprocessor, td.definition_name, token_vec);
}
...
int preprocessor_macro_function_push_something_definition(struct compile_process* compiler, struct preprocessor_definition* definition, struct preprocessor_function_arguments* arguments, struct token* arg_token, struct vector* definition_token_vec, struct vector* value_vec_target)
{
  ...
  // We failed so there is no argument
...
void preprocessor_handle_concat_part(struct compile_process* compiler, struct preprocessor_definition* definition, struct preprocessor_function_arguments* arguments, struct token* token, struct vector* definition_vector_vec, struct vector* value_vec_target)
{
  preprocessor_macro_function_push_something(compiler, definition, arguments, token, definition_vector_vec, value_vec_target);
}
...
void preprocessor_handle_concat(struct compile_process* compiler, struct preprocessor_definition* definition, struct preprocessor_function_arguments* arguments, struct token* arg_token, struct vector* definition_token_vec, struct vector* value_vec_target)
{
  ...
  if (!right_token)
  {
    compiler_error(compiler, "No right operand provided for concat preprocessor operatir ##");
  }
...
int preprocessor_macro_function_execute(struct compile_process* compiler, const char* function_name, struct preprocessor_function_arguments* arguments, int flags)
{
  ...
  {
    compiler_error(compiler, "The definition %s is not a macro function", function_name);
  }
...
{
  const char* macro_func_name = node->exp_node.left_node->sval;
  struct preprocessor_node* call_arguments = node->exp_node.right_node->parenthesis_node.exp;
...
void preprocessor_handle_definition_token(struct compile_process* compiler)
{
  ...
  struct preprocessor* preprocessor = compiler->preprocessor;
  preprocessor_definition_create(preprocessor, name_token->sval, value_token_vec, arguments);
}
