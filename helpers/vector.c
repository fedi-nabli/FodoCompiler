...
struct vector* vector_clone(struct vector* vector)
{
  ...
  // Saves are not cloned with vector_clone
...
{
  // Zero elements as we want to see if wa have already overflowed the index
...
void vector_save(struct vector* vector)
{
  ...
  // We are not allowed to modify the save's so set it to NULL
...
void vector_shift_right(struct vector* vector, int index, int amount)
{
  ...
  // let's stretch the vector up to index+amount
...
{
  // Popping at a peek is an awkward one
