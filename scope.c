...
void* scope_last_entity_from_scope_stop_at(struct scope* scope, struct scope* stop_scope)
{
  ...
  if (parent)
  {
    scope_last_entity_from_scope_stop_at(parent, stop_scope);
  }
...
{
  struct scope* new_current_scope = proces->scope.current->parent;
  scope_dealloc(proces->scope.current);
  proces->scope.current = new_current_scope;
  if (proces->scope.root && !proces->scope.current)
  {
    proces->scope.root = NULL;
  }
}
