...
struct resolver_entity* resolver_make_entity(struct resolver_process* process, struct resolver_result* result, struct datatype* custom_dtype, struct node* node, struct resolver_entity* guided_entity, struct resolver_scope* scope)
{
  ...
  switch (node->type)
  {
    case NODE_TYPE_VARIABLE:
      entity = resolver_create_new_entity_for_var_node_no_push(process, scope, node, NULL, offset);
      break;

    default:
      entity = resolver_create_new_entity_for_unknown_entity(process, result, custom_dtype, scope, node, offset);
  }
...
struct resolver_entity* resolver_create_new_entity_for_function_call(struct resolver_process* process, struct resolver_result* result, struct resolver_entity* left_operand_entity, void* private)
{
...
struct resolver_entity* resolver_get_entity_in_scope_with_entity_type(struct resolver_process* resolver, struct resolver_result* result, struct resolver_scope* scope, const char* entity_name, int entity_type)
{
  // Are we accessing a structure or union
  if (result && result->last_struct_union_entity)
  {
    struct resolver_scope* struct_union_scope = result->last_struct_union_entity->scope;
  ...
  // Dealing with a primitive datatype
...
struct resolver_entity* resolver_get_entity_for_type(struct resolver_process* resolver, struct resolver_result* result, const char* entity_name, int entity_type)
{
  struct resolver_scope* scope = resolver->scope.current;
  struct resolver_entity* entity = NULL;
  while (scope)
  {
    entity = resolver_get_entity_in_scope_with_entity_type(resolver, result, scope, entity_name, entity_type);
    if (entity)
    {
      break;
    }
...
struct resolver_entity* resolver_get_entity(struct resolver_process* resolver, struct resolver_result* result, const char* entity_name)
{
  return resolver_get_entity_for_type(resolver, result, entity_name, -1);
}

struct resolver_entity* resolver_get_entity_in_scope(struct resolver_process* resolver, struct resolver_result* result, struct resolver_scope* scope, const char* entity_name)
{
  return resolver_get_entity_in_scope_with_entity_type(resolver, result, scope, entity_name, -1);
}

struct resolver_entity* resolver_get_variable(struct resolver_process* resolver, struct resolver_result* result, const char* var_name)
{
...
void resolver_build_function_call_arguments(struct resolver_process* resolver, struct node* argument_node, struct resolver_entity* root_func_cal_entity, size_t* total_size_out)
{
  if (is_argument_node(argument_node))
  {
    // Build the left node for this function argument
    resolver_build_function_call_arguments(resolver, argument_node->exp.left, root_func_cal_entity, total_size_out);
    // Build the right node for this function argument
    resolver_build_function_call_arguments(resolver, argument_node->exp.right, root_func_cal_entity, total_size_out);
  }
  else if (argument_node->type == NODE_TYPE_EXPRESSION_PARENTHESES)
  {
    resolver_build_function_call_arguments(resolver, argument_node->parenthesis.exp, root_func_cal_entity, total_size_out);
  }
  else if (node_is_valid(argument_node))
  {
    vector_push(root_func_cal_entity->func_call_data.arguments, &argument_node);
...
struct resolver_entity* resolver_follow_function_call(struct resolver_process* resolver, struct node* node, struct resolver_result* result)
{
  ...
  struct resolver_entity* func_call_entity = resolver_create_new_entity_for_function_call(resolver, result, left_entity, NULL);
...
struct resolver_entity* resolver_follow_parentheses(struct resolver_process* resolver, struct node* node, struct resolver_result* result)
{
  ...
  return resolver_follow_exp(resolver, node, result);
}
...
struct resolver_entity* resolver_follow_indirection(struct resolver_process* resolver, struct node* node, struct resolver_result* result)
{
  ...
  struct resolver_entity* unary_indirection_entity = resolver_create_new_unary_indirection_entity(resolver, result, node->unary.operand, node->unary.indirection.depth);
...
struct resolver_entity* resolver_follow_unary_address(struct resolver_process* resolver, struct node* node, struct resolver_result* result)
{
  ...
  resolver_follow_part(resolver, node->unary.operand, result);
  struct resolver_entity* last_entity = resolver_result_peek(result);
  struct resolver_entity* unary_address_entity = resolver_create_new_unary_get_address_entity(resolver, result, &last_entity->dtype, last_entity->scope, node);
...
struct resolver_entity* resolver_follow_part_return_entity(struct resolver_process* resolver, struct node* node, struct resolver_result* result)
{
  ...
  switch (node->type)
  {
    ...
    default:
    {
      // Can't do anything, let's create a special entity that requires more computation later on at runtime
...
struct resolver_entity* resolver_merge_compile_times_result(struct resolver_process* resolver, struct resolver_result* result, struct resolver_entity* left_entity, struct resolver_entity* right_entity)
{
...
void resolver_finalize_result_flags(struct resolver_process* resolver, struct resolver_result* result)
{
  ...
  struct resolver_entity* entity = result->root_entity;
